...
class GeneticAlgorithmInnovative(Wrapper):
    ...
    def search(self, experiment_name, fitness_function, initial_population):
        ...
        while spent_nfes < self.max_nfes:
            ...
            for idx, new_fitness in enumerate(generated_population_fitness):
                fitness_solutions_indices = np.where(generated_population_fitness == new_fitness)[0]
                union = np.array([False] * len(generated_population[0]), dtype=bool)
                for j in fitness_solutions_indices:
                    union = union + np.array(generated_population[j]).astype(bool)

                union_fitness = fitness_function.evaluate_on_validation(union, comprehensive=False)

                if not any(np.array_equal(archive_element, union) for archive_element in archive) and \
                        union_fitness > np.min(archive_fitness):
                    worst_idx = np.argmin(archive_fitness)
                    archive[worst_idx] = copy.deepcopy(union)
                    archive_fitness[worst_idx] = union_fitness
