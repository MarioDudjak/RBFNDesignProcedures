...
for dataset in datasets:
    ...
    filename3 = 'RBFN-PSO-Fixed-' + str(iterations_auto) + '-Search-' + dataset + "-Performances"
    ...
    plt.xticks(np.arange(0, no_k_values * iterations_auto, step=iterations_auto), [str(i) for i in range(0, no_k_values)])
    plt.tick_params()
    plt.title('RBFN-PSO6-' + str(iterations_auto) + '-' + '-'.join(dataset.split('_')))
    legend = [r"PSO-Fixed"]
    ...
    plt.savefig('_'.join([dataset, 'RBFN_PSO_Fixed_MSE_convergence', str(iterations_auto)]) + ".pdf", format='pdf', dpi=300)
    plt.close()
