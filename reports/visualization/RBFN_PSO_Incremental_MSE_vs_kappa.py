...
for dataset in datasets:
    ...
    filename9 = 'RBFN-PSO-Incremental-Smart1-' + str(iterations-100) + '100-Search-' + dataset + '-CSharp'
    ...
    if header9 is not None and data9 is not None:
        ...
        data9 = data9[0:-1]
        for i in range(no_k_values):
            best_median_scores = [float(row[-1]) for j, row in enumerate(data9) if j % no_k_values == i]
    ...
    plt.plot(best9[dataset], lw=0.75, ms=4, marker=markers[9], color=colors[9])

    plt.ylabel('MSE', fontsize=13)
    plt.tick_params()
    plt.title('RBFN-PSO-Incremental $(N=30, c1=c2=1.468, w=0.724)$ -' + '-'.join(dataset.split('_')))
    #legend = [r"PSO-Incremental-Random1", r"PSO-Incremental-Random2", r"PSO-Incremental-Kmeans1", r"PSO-Incremental-Kmeans2", r"PSO-Incremental-FixedKmeans1", r"PSO-Incremental-FixedKmeans2"]
    legend = [r"PSO-Incremental-Random1", r"PSO-Incremental-Kmeans1", r"PSO-Incremental-FixedKmeans1", r"PSO-Incremental-Traditional1", r"PSO-Incremental-Mixed1", r"PSO-Incremental-Premature1"]
    plt.legend(labels=legend, fancybox=False, framealpha=0.9)
    plt.tight_layout()
    plt.grid(b=True, linestyle=':')
    #plt.show()
    plt.savefig('_'.join([dataset, 'RBFN_PSO_Incremental_MSE_vs_kappa_random_widths_versions']) + ".pdf", format='pdf', dpi=300)
    plt.close()
