...
pso = {}
pso_s = {}
datasets = ['Breastcancer', 'Climate', 'German_credit', 'Uci_Muskv1',
            'Ionosphere', 'Uci_imageseg', 'Wine', 'HillValley', 'Biodegradeable', 'Uci_parkinsons', 'Uci_voice',   'Urbanland', 'Heart-statlog', 'Hepatitis',   'Vote',
            'Connectionistbench', 'Wisconsin-breast-cancer', 'Clean2', 'glass']
...
for dataset in datasets:
    ...
    if header1 is not None and data1 is not None:
        ...
        for i in range(no_k_values):
            ...
            random[dataset].append(np.std(best_avg_scores))
    ...
    if header2 is not None and data2 is not None:
        ...
        for i in range(no_k_values):
            ...
            maxsse[dataset].append(np.std(best_avg_scores))
    ...
    if header3 is not None and data3 is not None:
        ...
        for i in range(no_k_values):
            ...
            nm[dataset].append(np.std(best_avg_scores))
    ...
    if header4 is not None and data4 is not None:
        ...
        for i in range(no_k_values):
            ...
            nm2[dataset].append(np.std(best_avg_scores))
    ...
    if header5 is not None and data5 is not None:
        ...
        for i in range(no_k_values):
            ...
            maxssemin[dataset].append(np.std(best_avg_scores))
    ...
    if header6 is not None and data6 is not None:
        ...
        for i in range(no_k_values):
            ...
            nm3[dataset].append(np.std(best_avg_scores))
    ...
    if header7 is not None and data7 is not None:
        ...
        for i in range(no_k_values):
            ...
            nm0[dataset].append(np.std(best_avg_scores))
    ...
    if header8 is not None and data8 is not None:
        ...
        best_k_scores = [float(row[-2]) for row in data8]
        de[dataset] = 20 * [np.std(best_avg_scores)]
    ...
    if header9 is not None and data9 is not None:
        ...
        pso[dataset] = 20 * [np.std(best_avg_scores)]

    if header10 is not None and data10 is not None:
        data10 = [row for row in data10 if row]
        data10 = data10[1:]
        for i in range(no_k_values):
            best_avg_scores = [float(row[-7]) for j, row in enumerate(data10) if j % no_k_values == i]
            pso_s[dataset].append(np.std(best_avg_scores))
