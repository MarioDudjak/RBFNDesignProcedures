...
for dataset in datasets:
    ...
    if header3 is not None and data3 is not None:
        ...
        for i in range(0, len(data1), no_k_values):
            best_random = max([float(row[-7]) for j, row in enumerate(data1) if j >= i and j < i + no_k_values])
            best_randoms.append(best_random)
            best_random_tpr = [float(row[4]) for j, row in enumerate(data1) if j >= i and j < i + no_k_values][
                [float(row[-7]) for j, row in enumerate(data1) if j >= i and j < i + no_k_values].index(best_random)]
            best_random_k = [round(float(row[-7]), 2) for j, row in enumerate(data1) if
                             j >= i and j < i + no_k_values].index(round(best_random, 2)) + 2
            best_random_ks.append(best_random_k)
            best_maxse = max([float(row[-7]) for j, row in enumerate(data2) if j >= i and j < i + no_k_values])
            best_maxses.append(best_maxse)
            best_maxse_tpr = [float(row[4]) for j, row in enumerate(data2) if j >= i and j < i + no_k_values][
                [float(row[-7]) for j, row in enumerate(data2) if j >= i and j < i + no_k_values].index(best_maxse)]

            best_maxse_k = [round(float(row[-7]), 2) for j, row in enumerate(data2) if
                            j >= i and j < i + no_k_values].index(round(best_maxse, 2)) + 2
            ...
            best_nm_k = [round(float(row[-7]), 2) for j, row in enumerate(data3) if
                         j >= i and j < i + no_k_values].index(round(best_nm, 2)) + 2
            best_nm_ks.append(best_nm_k)
            best_nm_tpr = [float(row[4]) for j, row in enumerate(data3) if j >= i and j < i + no_k_values][
            ...
            best_randoms_tprs.append(best_random_tpr)
            best_maxses_tprs.append(best_maxse_tpr)
            best_nms_tprs.append(best_nm_tpr)

        results[dataset].append([dataset, round(np.average(best_randoms), 2), round(np.std(best_randoms), 2),
                                 round(np.average(best_random_ks), 2), round(np.std(best_random_ks), 2),
                                 round(np.average(best_randoms_tprs), 2), round(np.std(best_randoms_tprs), 2),
                                 round(np.average(best_maxses), 2), round(np.std(best_maxses), 2),
                                 round(np.average(best_maxse_ks), 2), round(np.std(best_maxse_ks), 2),
                                 round(np.average(best_maxses_tprs), 2), round(np.std(best_maxses_tprs), 2),
                                 ...
                                 round(np.average(btrs), 2), round(np.average(btrmes), 2)])

CsvProcessor().save_summary_results(
    filename="F1_avg_test",
