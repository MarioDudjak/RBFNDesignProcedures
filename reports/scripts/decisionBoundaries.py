...
for k in range(2, 11):
    for file in datasets:
        ...
        if centers_header is not None and centers_data is not None:
            ...
            plot_decision_boundaries(X_train, y_train,  ax1, RBFN, centers=centers, widths=widths)
            plot_centers(ax1, centers, widths)
            ax1.set_title('Fixed-10000')

        centers_file = 'Incremental-Random1-500-' + file + '-Centers'
        centers_header, centers_data = CsvProcessor().read_file(filename='results/' + centers_file)
        if centers_header is not None and centers_data is not None:
            centers = []
            widths = []
            ...
            plot_decision_boundaries(X_train, y_train, ax2, RBFN, centers=centers, widths=widths)
            plot_centers(ax2, centers, widths)
            ax2.set_title('Incremental-Random1-500')

        centers_file = 'Incremental-KmeansFixed2-500-' + file + '-Centers'
        centers_header, centers_data = CsvProcessor().read_file(filename='results/' + centers_file)
        if centers_header is not None and centers_data is not None:
            centers = []
            widths = []
            ...
            plot_decision_boundaries(X_train, y_train, ax3, RBFN, centers=centers, widths=widths)
            plot_centers(ax3, centers, widths)
            ax3.set_title('Incremental-KmeansFixed2-500')

        centers_file = 'RBFN-PSO-Incremental-Smart1-400100-Search-' + file + '-Centers'
        centers_header, centers_data = CsvProcessor().read_file(filename='results/' + centers_file)
        if centers_header is not None and centers_data is not None:
            centers = []
            widths = []
            ...
            plot_decision_boundaries(X_train, y_train, ax4, RBFN, centers=centers, widths=widths)
            plot_centers(ax4, centers, widths)
            ax4.set_title('Incremental-Smart1-400100')
