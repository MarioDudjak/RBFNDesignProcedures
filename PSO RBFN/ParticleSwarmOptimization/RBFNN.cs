...
namespace ParticleSwarmOptimization
{
	class RBFNN
	{
		...
		public double[] calcOutput(double[] inputVector)
		{
			double[] outputVector = new double[nClasses];
			double hSum = 1E-5; //0.0;
		...
		public void PInvWeights()
		{
			...
			//int m = data.Size;
			//double[,] H = h.Transpose<double>();
			//H = H.MultiplyByTranspose(H);
            /*
			if (H.Determinant() == 0)
                Console.Write(".");
			...
			if (H.IsSingular())
				Console.Write("SSS");
			*/
            //if (Matrix.PseudoDeterminant (h)== 0)
            //{
            //    //Console.WriteLine("Singularity");
            //}
            //Console.WriteLine("->");
            //Console.WriteLine(h.GetLength(0) + ", " + h.GetLength(1));
